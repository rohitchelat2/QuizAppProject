...
{#if i>currentQuestion}
<QuizCard {question} {i} zin={questions.length-i} left={80-((rightCards*(questions.length-i))+1)} height ={1} width = {1}  activeQuestion = {true}  {validate} {editMode} bind:editQuestion bind:editQuestionIndex bind:renderSnippet {deleteQuestion}/>
{/if}
